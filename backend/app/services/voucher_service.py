...
class VoucherService(BaseService[Voucher]):
    ...
    async def _release_allocations(self, voucher: Voucher) -> None:
        ...
        The payment rows are untouched — the shop received that cash and the
        record of receiving it is not this bill's to destroy. Only the link
        goes, which turns the money into an advance sitting on the account.
        """
        ...
        for alloc in rows:
            payment = (
                await self.db.execute(
                    select(Payment).where(Payment.id == alloc.payment_id)
                )
            ).scalar_one_or_none()
            if payment is not None:
                payment.allocated_amount = money(payment.allocated_amount - alloc.amount)
                payment.unallocated_amount = money(
